...
public class Inventory : UdonSharpBehaviour {
    ...
    public Transform disconnectSyncTransform;
    ...
    [NonSerialized] Transform holsterParent;
    [NonSerialized] bool lateJoiner = true;

    [NonSerialized] public int handLayer;
    [NonSerialized] public bool trackHands;
    [NonSerialized] public GameObject rightHand;
    [NonSerialized] TrackingDataType headTracking = TrackingDataType.Head;
    [NonSerialized] TrackingDataType rightHandTracking = TrackingDataType.RightHand;
    [NonSerialized] TrackingDataType leftHandTracking = TrackingDataType.LeftHand;

    [NonSerialized] bool usedInventory;
    [NonSerialized] bool[] localPickupArrayHolstered;
    [NonSerialized] ushort localPickupHolsteredCount;
    ...
    [HideInInspector] public VRCObjectSync[] pickupArrayVRCObjectSync;
    ...
    public void _SetItemState(ushort _pickupId, bool _pickupWasHolstered) {
        ...
        usedInventory = true;
        SendCustomNetworkEvent(NetworkEventTarget.All, nameof(_SyncItemState), localPlayer.playerId, _pickupId, _pickupWasHolstered);
    }

    [NetworkCallable]
    public void _SyncItemState(int _playerId, ushort _pickupId, bool _pickupWasHolstered) {
        ...
        _SetItemHolstered(_pickupId, _pickupWasHolstered, false);

        RequestSerialization();
    }

    public void _SetItemHolstered(ushort _pickupId, bool _pickupWasHolstered, bool _respawnItems) {
        ...
        if (_pickupWasHolstered) {
            ...
            if (pickupsCanRespawn && _respawnItems) {
                VRCObjectSync objectSync = pickupArrayVRCObjectSync[_pickupId];
    ...
    public void _DropInventory() {
        ...
        SendCustomNetworkEvent(NetworkEventTarget.All, nameof(_SyncDropInventory), _pickups); //new DataToken(_pickups)
    }
    ...
    public override void OnPlayerLeft(VRCPlayerApi player) {
        ...
        Vector3 playerPos = player.GetPosition() + Vector3.up * 0.25f;
        ...
        for (ushort i = 0; i < pickupCount; i++) {
            ...
            pickupHolsteredCount = (ushort)Mathf.Max(pickupHolsteredCount - 1, 0);
            _DisconnectItemUnholster(i, respawnItemsOnDisconnect, playerPos);
        }

        RequestSerialization();
    }

    public void _DisconnectItemUnholster(ushort _pickupId, bool _respawnItems, Vector3 _playerPos) {
        if (_pickupId < 0 || _pickupId >= pickupCount) return;

        localPickupArrayHolstered[_pickupId] = false;
        localPickupHolsteredCount = (ushort)Mathf.Max(localPickupHolsteredCount - 1, 0);
        ...
        if (pickupsCanRespawn && _respawnItems) {
            ...
            if (objectSync) {
                disconnectSyncTransform.position = _playerPos;
                objectSync.TeleportTo(disconnectSyncTransform);
            } else
                _pickup.transform.position = _playerPos;
        }
    }
    ...
    public override void OnDeserialization() {
        if (lateJoiner) {
            for (ushort i = 0; i < pickupCount; i++) {
                ...
                _SetItemHolstered(i, true, false);
            }
        ...
        if (pickupHolsteredCount != localPickupHolsteredCount) { // Desync fix
            for (ushort i = 0; i < pickupCount; i++) {
                ...
                _SetItemHolstered(i, wasHolstered, false);
            }
        }
    }
