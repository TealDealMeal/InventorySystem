...
public static class SetupInventory {
    public static void Setup() {
        ...
        for (ushort i = 0; i < pickupArray.Length; i++) {
            ...
            if (((1 << _pickup.gameObject.layer) & inventory.holsterableItemLayers) == 0)
                continue;
        ...
        inventory.pickupsCanRespawn = inventory.respawnItemsOnDisconnect == true || inventory.respawnItemsOnRespawn == true;
        inventory.pickupArrayVRCObjectSync = new VRCObjectSync[pickupCount];
        if (inventory.pickupsCanRespawn)
        ...
        for (ushort i = 0; i < pickupCount; i++) {
            ...
            if (objectSync)
                inventory.pickupArrayVRCObjectSync[i] = objectSync;
            else if (inventory.pickupsCanRespawn)
        ...
        Debug.Log("[Inventory] Setup Finished");
    }
}
